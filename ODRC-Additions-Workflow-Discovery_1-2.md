...
### Repo-Aware Artifact Placement (New Section)

- DECISION: CC must validate repo state before placing any Chat-produced artifact. CC checks the target file(s) against current repo state and blocks placement if Code has modified those files since the artifact was generated. CC will not overwrite newer work from Code. This is not optional or advisory — it is a hard gate. The artifact either lands cleanly or CC rejects it and tells the developer why.

- OPEN: When CC blocks a placement due to stale or conflicting state, what's the recovery path? Options:
  1. CC shows the developer what changed and they manually reconcile.
  2. CC regenerates the artifact request with current state context so Chat can produce an updated version.
  3. CC routes the artifact to Code with instructions to integrate it into the current codebase.
  
  These may all be valid depending on the situation. Should CC recommend one based on the type of conflict?

- OPEN: Repo state validation requires CC to read current file state from GitHub. CC already has GitHub API access for deploys. The new requirement is comparing file timestamps/hashes between the Chat artifact's generation point and the current repo HEAD. What metadata does CC need to track — last-known commit SHA when the artifact was produced, file-level hashes, or something simpler?
