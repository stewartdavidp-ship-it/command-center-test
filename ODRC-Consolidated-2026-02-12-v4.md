...
# Updated: 2026-02-12 v4 — Phase 3 + bug fix complete, test suite building
...
29. **CC checks recent commits against completion file SHAs during existing polls.** One extra API call fetching 30 recent commits. Filters: ignore commits only touching `cc/` directory (prevents self-referential detection), ignore merge commits, configurable lookback window (default 14 days). Same dialog pattern: Dismiss, Reconstruct via Code, Ignore Permanently. SHA matching uses prefix matching to handle abbreviated vs full SHAs.

30. **Reconstruction via Code.** CC generates a markdown task document listing orphaned commit SHAs, dates, messages, and files changed. Copied to clipboard with download fallback. Code uses `git show`, file inspection, and CLAUDE.md context to produce proper completion files. Files land in `cc/completions/` and flow through normal ingestion. Reconstructed files are inherently lossy — narrative sections may be incomplete.

31. **Orphan auto-matching on completion file ingestion.** When a new completion file is ingested and its commit SHAs match existing orphan records, those orphans are automatically updated to "reconstructed" state. Matching happens during normal polling — no separate process needed.

### Multi-Phase Spec Packaging

32. **Chat creates spec packages when work exceeds one Code session.** Package contains a manifest (JSON) + ordered CLAUDE.md files. Manifest defines job sequence and dependencies. CC ingests package, creates job queue.
...
34. **`cc/tests/` is the third repo directory.** Repo structure: `cc/completions/` (Code output), `cc/specs/` (archived CLAUDE.md specs), `cc/tests/` (test infrastructure, fixtures, results). Test fixtures and specs live alongside test results.

35. **Code runs tests as part of post-task obligations.** Test execution happens between code commit and completion file generation. Results committed to `cc/tests/results/` as structured JSON. Completion files include an optional `tests` frontmatter section with pass/fail/skip counts and path to results file.

36. **Test results are informational, not a gate.** Consistent with journal-of-record philosophy. CC displays test summary badges on Job History cards. Chat validation bundles include test results. CC does not block packaging or progression based on test failures.

37. **Playwright with mock service layers for CC testing.** MockFirebaseDb, MockGitHubAPI, MockClaudeAPIService replace real services with in-memory equivalents. Test server injects mocks before app code runs. Minimal app changes — one `IS_TEST_MODE` flag check and `data-testid` attributes.

### Task Tracking & Classification

38. **Completion files support optional `task_type` field.** Values: `bug_fix`, `feature`, `refactor`, `chore`, `tech_debt`, `hotfix`. Enables CC to filter, aggregate, and eventually export by type. Unplanned work without task_type gets classified after the fact through Chat.

39. **Completion files support optional `tracking` section.** Contains: `type` (same as task_type), `source_spec` (cc-spec-id), `external_ref` (Jira ticket, GitHub issue, etc.), `priority` (low/medium/high/critical). This is the bridge to external systems.

40. **Bug fix specs are lightweight CLAUDE.md files.** No phased approach, no architecture rules section, no "What NOT to Build." Just: what's broken, why, what to change, where to look, and post-task obligations. Uses `cc-spec-id: bug_{short_id}` naming. Same completion file output requirement — bugs produce completion files like everything else.

### API Key Separation

41. **Anthropic API key is stored separately from CC internal token.** `cc_anthropic_api_key` in localStorage holds the real Anthropic API key (`sk-ant-...`) for ClaudeAPIService. `cc_api_key` remains the CC-generated internal token for deploy queue authentication. Settings view has a dedicated input section with password field, show/hide toggle, status indicator, and link to console.anthropic.com. Key added to StorageManager.PROTECTED_KEYS.

### Ideation Platform (Steps 1-4)

42. **ConceptManager and IdeaManager are standalone JS objects** (not React components) placed between GitHub API class and Main App function. They are the data/service layer.

43. **`appIdeas/{appId}` is a denormalized Firebase index.** Maps apps to idea IDs for fast lookup. Source of truth is `ideas/{ideaId}`.

44. **`supersede()`, `resolve()`, and `transition()` are distinct operations** on concepts. Supersede replaces content (same type). Resolve marks an OPEN as done. Transition changes type following the state machine.

45. **IdeasView is its own top-level nav entry** under the Plan dropdown, with three modes (All Concepts, App Aggregate, Idea Detail) and relationship links from Dashboard and Backlog.

46. **Top-level listeners for `globalConcepts` and `globalIdeas`** in App component, following existing pattern for all shared data.
...
3. **One active Code job per repo at a time.** CC enforces sequential execution to prevent merge conflicts. (See Decision 33.)
...
8. What is the exact structured output format Chat should use for ODRC recommendations? Needs a formal schema for reliable CC parsing. (Phase 3 implements a parser but format may need tuning as real Chat output patterns emerge.)

9. How does the developer get Chat's structured output back into CC? Phase 3 implements copy-paste into the Import modal. File upload? Direct API integration?
...
15. The codebase is now ~24K+ lines and growing. Should satellite extraction be prioritized?

### Rule Inheritance (Future Feature)

16. CC should auto-populate base RULEs and CONSTRAINTs when an app is created, based on stack tags. Three tiers: Universal, Stack-specific, App-specific. When should this be built?
...
21. Orphan commit rate limiting: polling 30 commits + detail fetch per commit could hit GitHub API rate limits for repos with frequent commits. Should we add commit-level caching?

### Multi-Phase Spec Packaging

22. When should the multi-phase spec packaging and job queue feature be built? Not needed for current manual phasing but becomes important as workflow matures.

23. Manifest format — JSON or markdown? Current lean: JSON (easier to parse).

24. How does CC ingest the spec package? Developer uploads zip to CC? CC detects it in a repo directory? Chat pushes via some mechanism?

### API & Infrastructure

25. ClaudeAPIService uses `anthropic-dangerous-direct-browser-access` header for browser-based API calls. Is this the long-term pattern or should calls go through Firebase Functions?

26. Review prompt quality: using Haiku for cost efficiency. Should there be a model selector in settings for users who want higher quality prompts?

27. The js-yaml CDN adds a new external dependency. Should we vendor it or keep CDN? CDN is consistent with existing React/Firebase/JSZip pattern.

### Test Infrastructure

28. How should CC handle test result files in `cc/tests/results/` that don't match any completion file? Orphan test results could indicate an incomplete Code session.

29. Should test failure change the urgency of Chat validation? (e.g., auto-suggest "Package for Check" when a completion file reports failed tests)

30. Test trend analytics — pass rate over time, test count growth, flaky test detection. When does this become valuable enough to build?

31. Should CC validate that tests were actually run? (completion file claims 12/12 passed but no results file exists in `cc/tests/results/`)

### Task Tracking

32. Should `task_type` and `tracking` be formally added to the completion file spec? (Currently optional/informal.)

33. How does CC's Job History display and filter by task type?

34. External system integration (Jira, GitHub Issues) — what does export/sync look like?

### Bug Fix Workflow

35. How does CC generate and dispatch bug fix specs? Quick-action from Dashboard? Button in Job History when a validation challenge is recorded? What's the UI flow?

### ODRC Ingestion Refinement

36. ODRC parser error tolerance: the parser handles common variations (arrow styles, quotes), but Chat output may have more diverse formats. Should we add a "manual entry" fallback for unparseable lines?

37. Reconstruction task delivery: currently clipboard-only. Should we add repo push to `cc/tasks/` so Code can pick it up automatically?

38. Batch classification size: no limit on number of jobs in a batch. For large batches, the Claude API call may exceed token limits. Should we add batching within batches?
...
| (P1 new) | listRepoContents/getFileContent method names | → Verified: methods exist on GitHubAPI class |

*Resolved during Phase 3 build (2026-02-12):*

| Original # | Question | Resolution |
|------------|----------|------------|
| (P3 scope) | Orphan commit detection | → Decision 29-31: Full implementation with auto-matching |
| (P3 scope) | ODRC update ingestion from Chat | → Decision 23: Parser + confirmation checklist + executor |
| (P3 scope) | Batch classification packaging | → Batch flow with job selector, zip assembly, higher token budget prompt |

*Resolved during bug fix (2026-02-12):*

| Original # | Question | Resolution |
|------------|----------|------------|
| (bug) | ClaudeAPIService uses wrong API key | → Decision 41: Separate cc_anthropic_api_key with Settings UI |
...
cc/
  ...
  specs/           ← Archived CLAUDE.md specs (planned + bug fix)
...
| 8.60.0 | sp_ingest_p3 | feature | ✅ Complete | Orphan detection, ODRC ingestion, batch classification |
| 8.60.1 | bug_api_key_mismatch | bug_fix | ✅ Complete | Anthropic API key separation from CC internal token |
| — | sp_cc_tests | infrastructure | 🔨 Building | Playwright test suite with mock services |
...
**Decisions ratified:** 15 new decisions (Decisions 15-33), resolving 10 previously open items.

### Session 3: Pipeline Build, Test Infrastructure & Bug Fix

**Session origin:** Executing the three-phase build, designing test infrastructure, fixing API key bug.

**Key progression:** Phase 1 built (8.58.0) → Phase 2 built (8.59.0) → Phase 3 built (8.60.0) → API key bug discovered → bug fix spec pattern designed → bug fixed (8.60.1) → test infrastructure designed (Playwright + mocks) → test results integration into completion files → task_type and tracking fields designed → test suite dispatched to Code.

**Decisions ratified:** 13 new decisions (Decisions 34-46). New OPENs: 17 added across 7 categories. Resolved: 4 additional OPENs from build findings and bug fix.

**Pipeline milestones:**
- First completion file detected by CC: Phase 1's own completion file ✅
- First bug fix spec produced and executed: API key mismatch ✅
- First `task_type` and `tracking` fields used in practice: bug fix completion file ✅
- Full ingestion loop operational: Code → CC → bundle → Chat → ODRC ingestion ✅

**What's next:** Test suite build completes → run tests → first full end-to-end validation (package a completion file, validate in Chat, import ODRC updates back to CC).
